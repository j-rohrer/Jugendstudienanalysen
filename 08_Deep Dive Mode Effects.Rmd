...
```{r media_usage}
media_usage_gaming <- lm(gaming ~ source*gender*age*as.factor(schooltype)*as.factor(mig_lang), data = combined[combined$year == 2023,])
...
media_usage_streaming <- lm(streaming ~ source*gender*age*as.factor(schooltype)*as.factor(mig_lang), data = combined[combined$year == 2023,])
...
media_usage_socialmedia <- lm(socialmedia ~ source*gender*age*as.factor(schooltype)*as.factor(mig_lang), data = combined[combined$year == 2023,])
...
ggplot(data = comp_gaming, aes(y = estimate, x = age, group = gender, color = gender)) +
  geom_point() +
  geom_line() +
  geom_ribbon(alpha = .1, color = NA, aes(ymin = conf.low, ymax = conf.high, fill = gender)) +
  theme_classic()
...
## Duration
```{r time}
hist(combined$duration_seconds)
combined$duration_min <- combined$duration_seconds/60

duration <- lm(duration_min ~ gender*age*as.factor(schooltype)*mig_lang, data = combined[combined$year == 2023,])


predictions_duration <- avg_predictions(duration, variables = c("gender", "age"), vcov = ~ unique_classroom)

print(avg_comparisons(duration, variables = "gender", by = "age", vcov = ~ unique_classroom))



ggplot(data = predictions_duration, aes(y = estimate, x = age, group = gender, color = gender,
                                        ymin = conf.low, ymax = conf.high, fill = gender)) +
  geom_point() +
  geom_line() +
  geom_ribbon(alpha = .1, color = NA) +
  theme_classic()

# lets split this by schooltype because it looks wonky

predictions_duration_schooltype <- avg_predictions(duration, variables = c("gender", "age", "schooltype"), vcov = ~ unique_classroom)


ggplot(data = predictions_duration_schooltype[predictions_duration_schooltype$schooltype == 1,], aes(y = estimate, x = age, group = gender, color = gender,
                                        ymin = conf.low, ymax = conf.high, fill = gender)) +
  geom_point() +
  geom_line() +
  geom_ribbon(alpha = .1, color = NA) +
  ggtitle("General secondary") +
  theme_classic()

ggplot(data = predictions_duration_schooltype[predictions_duration_schooltype$schooltype == 2,], aes(y = estimate, x = age, group = gender, color = gender,
                                        ymin = conf.low, ymax = conf.high, fill = gender)) +
  geom_point() +
  geom_line() +
  geom_ribbon(alpha = .1, color = NA) +
...
```{r within_classroom}
classrooms <- unique(youth2023$unique_classroom)

for (i in 1:length(classrooms)) {
  print(table(youth2023$source[youth2023$unique_classroom == classrooms[i]]))
}

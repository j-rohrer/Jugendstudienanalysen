...
# Possible problems: splines to flexible
# empty cells in the average comparison?

mode_effects_controlled <- lm(satis ~ source*gender*as.factor(schooltype)*as.factor(mig_lang)*as.factor(age), data = combined[combined$year == 2023,])
...
mode_effects_controlled_fe <- lm(satis ~ source*gender*as.factor(schooltype)*as.factor(mig_lang)*bs(age, df = 3) +
...
```{r mode_explanandum}
explanandum_paper_only <- lm(satis ~ as.factor(year)*gender*as.factor(schooltype)*bs(age, df = 3), 
